      ...
      auto filterFunc =
          [&leftJoinIndices,
           ...
           stream](
            ...
            cudf::groupby::groupby keysleftIndicesColGrouper(keysLeftIndicesCol, cudf::null_policy::EXCLUDE, cudf::sorted::NO);
            ...
            auto groups = std::move(result.first);
            ...
            auto leftTableViewRowIndicesTable = cudf::apply_boolean_mask(groups->view(), uniqueFilter->view(), stream);
            ...
            auto left_extra_result = cudf::gather(
                leftTableView, extra_rows, oobPolicy, stream);
            ...
            return filteredLeftJoinTable->release();
          };
      */
      ...
      auto filterFunc =
          [&leftJoinIndices,
           ...
           stream](
            ...
            cudf::groupby::groupby keysleftIndicesColGrouper(cudf::table_view{{leftIndicesCol}}, cudf::null_policy::EXCLUDE, cudf::sorted::NO);
            std::vector<cudf::groupby::aggregation_request> requests;
            requests.emplace_back(cudf::groupby::aggregation_request{});
            requests.emplace_back(cudf::groupby::aggregation_request{});
            requests[0].aggregations.push_back(cudf::make_sum_aggregation<cudf::groupby_aggregation>());
            ...
            auto rowIndexColumn = cudf::sequence(leftIndicesCol.size(), zero_scalar, stream);
            ...
            auto filteredMinIndicesTable = cudf::apply_boolean_mask(cudf::table_view{{minIndices->view()}}, uniqueFilter->view(), stream);
            auto filteredMinIndices = filteredMinIndicesTable->get_column(0).view();
            {
              std::vector<std::unique_ptr<cudf::scalar>> nullScalars;
              std::vector<std::reference_wrapper<cudf::scalar const>> nullScalarRefs;
              std::vector<cudf::column_view> rightColViews;
              for (auto col = leftColsSize; col < joinedCols.size(); col++) {
                nullScalars.emplace_back(cudf::make_empty_scalar_like(joinedCols[col]->view(), stream));
                nullScalarRefs.push_back(std::reference_wrapper<cudf::scalar const>(*(nullScalars.back())));
                rightColViews.push_back(joinedCols[col]->view());
              }
              auto nullifiedRightTable = cudf::scatter(nullScalarRefs, filteredMinIndices, cudf::table_view(rightColViews), stream);
            ...
            std::vector<std::reference_wrapper<cudf::scalar const>> source_scalars{true_scalar};
            auto filteredColumnTable = cudf::scatter(source_scalars, filteredMinIndices, cudf::table_view{{filterColumn}}, stream);
