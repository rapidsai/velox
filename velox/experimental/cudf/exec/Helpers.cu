...
if (joinNode_->filter() && joinNode_->isInnerJoin()) {
  ...
  // for left join, we need to ensure that all rows in the left table exist
after the
  // filter is applied
  thrust::sort_by_key(rmm::exec_policy_nosync(stream), leftJoinIndices->begin(),
leftJoinIndices->end(), rightJoinIndices->begin()); auto leftResult =
      cudf::gather(leftTableView, leftIndicesCol, oobPolicy, stream);
  auto rightResult =
      cudf::gather(rightTableView, rightIndicesCol, oobPolicy, stream);
  auto leftColsSize = leftResult->num_columns();
  ...
  if (!(result->is_valid(stream) && result->value(stream))) {
    ...
    rmm::device_uvector<bool> unique_filter(leftTableView.num_rows(), stream,
mr); cudf::device_span<bool const> filter_column_span = filterColumn;
    thrust::reduce_by_key(rmm::exec_policy_nosync(stream),
leftJoinIndices->begin(), leftJoinIndices->end(), filter_column_span.begin(),
thrust::make_discard_iterator(), unique_filter.begin(), cuda::std::equal_to{},
        cuda::std::logical_or{});
    auto num_extra_rows = thrust::count_if(rmm::exec_policy(stream),
unique_filter.begin(), unique_filter.end(), [] __device__ (auto b) { return !b;
        });
...
thrust::counting_iterator(leftTableView.num_rows()), extra_rows.begin(),
    ...
    auto left_extra_result = cudf::gather(leftTableView,
cudf::column_view{extra_rows_span}, oobPolicy, stream); auto extra_columns =
left_extra_result->release(); for(auto col = 0; col < rightColsSize; col++) {
      auto null_scalar = cudf::make_empty_scalar_like(joinedCols[col]->view(),
stream);
      //auto res = cudf::copy_if_else(joinedCols[col]->view(), *null_scalar,
filterColumn, stream);
      extra_columns.push_back(cudf::make_column_from_scalar(*null_scalar,
num_extra_rows, stream, mr);
    }
    ...
    auto filteredLeftJoinTable = cudf::concatenate(concat_table_views, stream,
mr);
...
unique_filter.begin(), unique_filter.end(), [] __device__ (auto b) { return !b;
  ...
  // Identify rows from the left table that are false in unique_filter
  rmm::device_uvector<cudf::size_type> extra_rows(num_extra_rows, stream, mr);
  thrust::copy_if(rmm::exec_policy_nosync(stream), thrust::counting_iterator(0),
thrust::counting_iterator(leftTableView.num_rows()), extra_rows.begin(),
      [unique_filter = unique_filter.begin()] __device__(auto i) {
        return !unique_filter[i];
      });
